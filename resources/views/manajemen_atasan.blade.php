    ...
    <div class="relative min-h-screen lg:flex">
        <aside id="sidebar" class="bg-white w-64 flex-col fixed inset-y-0 left-0 transform -translate-x-full transition-transform duration-300 ease-in-out z-50 lg:relative lg:translate-x-0 lg:flex border-r border-slate-200">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2">
                ...
                <a href="{{ url('/admin/atasan') }}" class="flex items-center px-4 py-2.5 text-white bg-hijau-500 rounded-lg font-semibold"><i class="bi bi-person-badge-fill mr-3"></i> Manajemen Atasan</a>
        ...
        <main class="flex-1 p-4 md:p-8 overflow-y-auto">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-500 mt-1">Tambah dan kelola data atasan untuk ASN.</p>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($atasan as $a)
                                <tr class="atasan-row hover:bg-slate-50" data-nama="{{ strtolower($a->nama) }}" data-nip="{{ strtolower($a->nip) }}">
                                    <td class="p-4">
                                        <p class="font-semibold text-gray-800">{{ $a->nama }}</p>
                                        <p class="text-xs text-gray-500 md:hidden">NIP: {{ $a->nip }}</p>
                                    </td>
                                    <td class="p-4 text-sm text-gray-600 hidden md:table-cell">{{ $a->nip }}</td>
                                    <td class="p-4 text-sm text-gray-600 hidden lg:table-cell">{{ $a->jabatan }}</td>
                                    <td class="p-4 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ url('/admin/atasan/'.$a->id.'/hapus') }}" onclick="return confirm('Yakin ingin menghapus data atasan {{ $a->nama }}?')" class="text-gray-500 hover:text-red-600 p-2 rounded-md hover:bg-red-100"><i class="bi bi-trash-fill"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @empty
    ...
    <div id="modalAtasan" class="hidden custom-modal-backdrop">
        <div class="custom-modal-content">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-2xl font-bold text-gray-800">Tambah Data Atasan</h3>
                <button id="closeModalAtasanButton" class="text-gray-400 hover:text-gray-600 text-3xl">&times;</button>
            </div>
            
            <form action="{{ url('/admin/atasan') }}" method="POST">
                @csrf
                <div class="space-y-6">
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Nama Lengkap</label><input type="text" name="nama" class="w-full px-4 py-3 bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-hijau-400" required></div>
                ...
                <div class="flex justify-end gap-4 mt-8">
                    <button id="batalAtasan" type="button" class="px-6 py-3 bg-slate-200 text-gray-800 rounded-lg hover:bg-slate-300 font-semibold">Batal</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function closeModal() {
                ...
                setTimeout(() => modalAtasan.classList.add('hidden'), 300);
            }

            btnTambahAtasan.addEventListener('click', openModal);
            closeModalAtasanButton.addEventListener('click', closeModal);
            batalAtasan.addEventListener('click', closeModal);

            // Live Search Logic (Tanpa request ke server)
            const searchInput = document.getElementById('searchInput');
            const rows = document.querySelectorAll('.atasan-row');
